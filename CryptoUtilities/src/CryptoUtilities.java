...
public final class CryptoUtilities {
    ...
    /**
     * Constant for NaturalNumber one.
     */
    private static final int ONE = 1;

    /**
     * Constant for NaturalNumber two.
     */
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        // get the last digit and check if its even
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p, NaturalNumber m) {
        ...
        final NaturalNumber two = new NaturalNumber2(TWO);
        ...
        if (p.isZero()) {
            ...
            if (isEven(p)) {
                ...
                NaturalNumber t = new NaturalNumber2();
                t.copyFrom(n);
                n.multiply(t);
                ...
                NaturalNumber newN = new NaturalNumber2();
                newN.copyFrom(n);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w, NaturalNumber n) {
        ...
        NaturalNumber one = new NaturalNumber2(ONE);
        NaturalNumber two = new NaturalNumber2(TWO);
        ...
        return (wCopy.compareTo(one) == 0 || w.compareTo(one) != 0);
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            /*
             * random number range needs to be within (1, n-1) or [2, n-2]. This
             * can be written as [0, n-4] + 2
             */

            // n - 4
            NaturalNumber t = new NaturalNumber2(n);
            t.subtract(two);
            t.subtract(two);

            int i = 0;
            while (isPrime && i < numCandidates) {
                // add 2
                NaturalNumber rand = randomNumber(t);
                rand.add(two);

                if (isWitnessToCompositeness(rand, n)) {
                    isPrime = false;
                }
                i++;
            }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        /*
         * add one to make the even number odd since all even numbers are
         * composite
         */
        if (isEven(n)) {
            n.increment();
        ...
        // keep checking odd numbers until you find a prime number
        while (!isPrime2(n)) {
            n.add(two);
        }
    }
