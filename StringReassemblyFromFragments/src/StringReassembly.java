...
public final class StringReassembly {
    ...
    public static int overlap(String str1, String str2) {
        ...
        assert str2.indexOf(str1) < 0
                : "Violation of: " + "str1 is not substring of str2";
        assert str1.indexOf(str2) < 0
                : "Violation of: " + "str2 is not substring of str1";
        /*
    ...
    public static String combination(String str1, String str2, int overlap) {
        assert str1 != null : "Violation of: str1 is not null";
        assert str2 != null : "Violation of: str2 is not null";
        assert 0 <= overlap && overlap <= str1.length() && overlap <= str2.length()
                && str1.regionMatches(str1.length() - overlap, str2, 0, overlap)
                : "" + "Violation of: OVERLAPS(str1, str2, overlap)";

        String s = str1.substring(0, str1.length() - overlap) + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        // Check if str is a substring of any of the strings in the set
        boolean flag = true;
        // Use an iterator for efficiency (able to exit when
        // str is found to be a substrings
        Iterator<String> iter = strSet.iterator();
        while (flag && iter.hasNext()) {
            flag = flag && !iter.next().contains(str);
        }

        if (flag) {
            // Look for all strings that are a substring of str
            // and add them to a new set
            Set<String> removeSet = strSet.newInstance();
            for (String s : strSet) {
                if (str.contains(s)) {
                    removeSet.add(s);
                }
            }
            // remove the new set and add the new string
            strSet.remove(removeSet);
            strSet.add(str);
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // add the next line avoiding substrings
        while (!input.atEOS()) {
            String s = input.nextLine();
            System.out.println(s);
            addToSetAvoidingSubstrings(set, s);
        }
    ...
    public static void assemble(Set<String> strSet) {
        ...
        while ((strSet.size() > 1) && !done) {
            ...
            if (bestOverlap == 0) {
                ...
                String overlapped = combination(bestTwo[0], bestTwo[1], bestOverlap);
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // replace the separator '~' with a new line character and print
